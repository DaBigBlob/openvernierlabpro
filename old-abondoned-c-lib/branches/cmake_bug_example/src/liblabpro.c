        ...
        for (ssize_t i = 0; i < cnt && lp_list.num < 5; ++i) { // We won't support more than 5 connected LabPros at once, that would be ridiculous
            ...
            if (desc.idVendor == 0x08f7 && desc.idProduct == 1) {
                ...
                if ((open_err = libusb_open(usb_list[i], &dev_handle)) != LIBUSB_SUCCESS) {
                    ...
                    if ((interface_claim_error = libusb_claim_interface(dev_handle, 1)) != 0) {
                    ...
                    for (uint8_t i = 1; i <= config->interface[0].altsetting[0].bNumEndpoints; ++i) {
                        ...
                        {
                            printf("[liblabpro ERR] LabPro %d had unexpected non-bulk endpoint.\n", lp_list.num);
                            continue;
                        }
                         ...
                         */
                        unsigned char address = ep_desc.bEndpointAddress & 0b00001111;
                    ...
                    if (!in_endpt_found && !out_endpt_found) {
                        printf("[liblabpro ERR] No bulk endpoints found for LabPro %d.\n", lp_list.num);
                        libusb_close(dev_handle);
                        continue;
                    }
...
int LabPro_send_raw(LabPro* labpro, char* command, int* length_transferred) {
    ...
    if ((command = realloc(command, strlen(command) + 2)) == NULL)
        return LABPRO_ERR_NO_MEM;
    
    char* real_command = strcat(command, "\r");
    ...
    for (int i = 1; i <= numpackets; ++i) {
        ...
        *length_transferred += transferred;
        ...
        if (status != LIBUSB_SUCCESS) {
            ++numerrors;
            printf("[liblabpro ERR] Error writing to USB: %s", libusb_strerror(status));
            printf("[liblabpro WARN] There have been %d errors for this write function so far.", numerrors);
...
int LabPro_read_raw(LabPro* labpro, char** string, int* length) {
    ...
    int numpackets = 1; // Number of packets we've transferred so far.
    *length = 0;
    int numerrors = 0; // Number of times libusb has returned an error
    unsigned char* data = malloc(64 * sizeof(char)); // The data buffer
    if (data == NULL)
        return LABPRO_ERR_NO_MEM;
    ...
    do {
        ...
        if (status == LIBUSB_ERROR_NO_DEVICE) {
            LabPro_handle_device_disconnect(labpro);
            memset(data + (64 * numpackets), 0, 64); // NULL-terminate the string since we allocated this memory and didn't fill it.
        ...
        if (status != LIBUSB_SUCCESS && status != LIBUSB_ERROR_TIMEOUT) {
            ...
            printf("[liblabpro ERR] Error reading from USB: %s", libusb_strerror(status));
            printf("[liblabpro WARN] There have been %d errors for this read function call so far.", numerrors);
