...
enum LabPro_Errors {
    ...
    /** \brief LabPro_parse_list() was called with an incorrectly-formatted list. */
    LABPRO_ERR_BAD_LIST
};
...
/** \brief Struct describing the LabPro firmware version */
typedef struct {
    int product_code;
    int major;
    int minor;
    int step;
} LabPro_Firmware_Version;

/** \brief Holds information about connected sensors, etc.*/
typedef struct {
    enum LabPro_Channels channel_number;
    char* sensor_long_name;
    char* sensor_short_name;
} LabPro_Channel;
...
typedef struct {
     ...
     */
    LabPro_Firmware_Version firmware_version;
    
    /** \brief The LabPro's current error code.
     * liblabpro will try to avoid triggering any errors in the LabPro, but if any occur,
     * this number will be what command 7 returns.
     */
    int errorcode;
    
    /** \brief The LabPro's battery level. */
    enum LabPro_Battery_Level battery_level;
    
    /** \brief The last used temperature correction value for sonic channels.
     * Probably not that useful for a GUI application which can store this itself.
     */
    unsigned int last_temperature_correction;
    
    /** \brief Whether the LabPro's internal speaker is enabled. */
    bool sound_enabled;
    
    enum LabPro_System_Status system_status;
    
    LabPro_Channel analog_channel_1;
    LabPro_Channel analog_channel_2;
    LabPro_Channel analog_channel_3;
    LabPro_Channel analog_channel_4;
    LabPro_Channel sonic_channel_1;
    LabPro_Channel sonic_channel_2;
    LabPro_Channel digital_channel_1;
    LabPro_Channel digital_channel_2;
 ...
 *               Maybe useful if you're requesting binary sample data.
 ...
 * data. This replaces the trailing carriage return character
 * with a NULL character.
 * 
 * \param string The string to trim
 * \return Zero if the string was properly NULL-ified, 1 if a CR character wasn't found.
 * 
 * \ingroup internal
 */
int LabPro_trim_response(char* string);

/** \brief Parse TI OS-style list into an array of strings.
 * The string should already have been cleaned with LabPro_trim_response().
 * The closing '}' character may be followed by garbage data, provided
 * the data doesn't include a ','.
 * 
 * \param string The string representation of the list
 * \param argv_list Pointer to array of strings
 * \return One of \ref LabPro_Errors.
 * 
 * \ingroup internal
 */
int LabPro_parse_list(char* string, int* argc_list, char*** argv_list);

/** \brief Update the LabPro struct with the LabPro's current status
 */
